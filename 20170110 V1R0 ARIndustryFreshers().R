...
Current$Shift=NULL
Current$Shift2=NULL
IndustryFresher <- subset(Current, ExperienceType == 'Industry Fresher')

##################### Use limited data #############


dataset2 <- IndustryFresher[c(  "EmployeeCode", "EmployeeName", "AGSExperienceInMonths", "Gender", "EmployeeAge"
...
################# 4 Model 1 (all IMP) #######################
# + LastReviewType + LastReviewRating + RMonth + Shift3
set.seed(144)
...
(263+384)*100/(263+384+21+24) #  93.49711
################# 6 Export Model Score #################

t <- cbind(dataset2, predAll1)
t$Prediction <- ifelse(predAll1>0.5, "Leave", "Stay")
write.csv(t, file="ARIndustryFresher_Logit_Score_R0.csv", row.names=FALSE)