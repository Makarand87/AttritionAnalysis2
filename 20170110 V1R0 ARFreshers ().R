...
Current$Shift=NULL
Current$Shift2=NULL
Freshers <- subset(Current, ExperienceType == 'Fresher')

##################### Use limited data #############


dataset2 <- Freshers[c(  "EmployeeCode", "EmployeeName", "AGSExperienceInMonths", "Gender", "EmployeeAge"
...
(730+710)*100/(730+710+106+76) #  88.77928
################# 6 Export Model Score #################

t <- cbind(dataset2, predAll1)
t$Prediction <- ifelse(predAll1>0.5, "Leave", "Stay")
write.csv(t, file="ARFresher_Logit_Score_R0.csv", row.names=FALSE)
