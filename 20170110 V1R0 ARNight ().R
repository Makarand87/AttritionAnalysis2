...
Current$Shift=NULL
Current$Shift2=NULL
Night <- subset(Current, Shift3 == '06:00 PM-03:00 AM')

##################### Use limited data #############


dataset2 <- Night[c(  "EmployeeCode", "EmployeeName", "AGSExperienceInMonths", "Gender", "EmployeeAge"
...
################# 4 Model 1 (all IMP) #######################
# + Last30DaysLeaveCount+ TotalExtraHoursWorked + LastReviewType + LastReviewRating + RMonth + Shift3
set.seed(144)
logit1 <- glm(Attrition ~  EngagementIndex + AGSExperienceInMonths
              + QualAvgLast3Months + FunctionName + WorkLocation + CourseLevels
              + ExperienceType + ProdAvgLast3Months 
              + TransportMode + EmployeeAge + JMonth + Client + Last30DaysLeaveCount+ TotalExtraHoursWorked 
...
(1091+1250)*100/(1091+1250+120+156) #  89.45357

################# 6 Export Model Score #################
t <- cbind(dataset2, predAll1)
t$Prediction <- ifelse(predAll1>0.5, "Leave", "Stay")
write.csv(t, file="ARNight_Logit_Score_R0.csv", row.names=FALSE)
